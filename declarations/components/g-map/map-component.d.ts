...
export type PascalCase<S extends string> = S extends `${infer Head}_${infer Tail}` ? `${Capitalize<Head>}${PascalCase<Tail>}` : Capitalize<S>;
export type MapEvents<MapComponentInstance, EventMap extends Record<string, unknown>> = {
    [Name in keyof EventMap & string as `on${PascalCase<Name>}`]?: (event: MapEvent<EventMap[Name], MapComponentInstance>) => void;
} & {
    [Name in keyof EventMap & string as `onceOn${PascalCase<Name>}`]?: (event: MapEvent<EventMap[Name], MapComponentInstance>) => void;
};
