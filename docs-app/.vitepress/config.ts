...
// The Maps key is baked in from process.env at build time -- the deploy
// workflow supplies it as a CI secret. For local `pnpm --filter docs-app
// start`, also read it from the repo-root .env.test so you don't have to
// export it by hand. Skipped when the var is already set (CI) or the file
// is absent.
if (!process.env.GOOGLE_MAPS_API_KEY) {
  const repoRootEnv = resolve(dirname(fileURLToPath(import.meta.url)), '../../.env.test');
