...
// see https://github.com/warp-drive-data/warp-drive/blob/main/warp-drive-packages/ember/src/install.ts
// revisit things in here once the scheduler RFC has finalized: https://github.com/emberjs/rfcs/pull/957

// Do a setup effect's first pull
// A component's first setup is kicked off by the async Google Maps API load (a bare
// promise microtask, no runloop), so nothing would run that first pull. We instead
// add to the runloop and batch the pull with rendering. All later updates ride the
// poll as before.
function scheduleInitialPull(effect) {
  schedule('afterRender', () => getValue(effect));
}
...
export { scheduleInitialPull, setupEffect };
