...
// v2 delta (was `['lat', 'lng', 'getContext']`): the v1 `getContext` arg no
// longer exists — context is ambient via the facade, not passed as an arg.
const ignoredOptions = ['lat', 'lng'];
...
function addEventListener(target, originalEventName, action, payload = {}) {
  ...
  let addListener = isDom && target?.nodeName !== 'GMP-INTERNAL-AM' && target?.nodeName !== 'GMP-ADVANCED-MARKER' ? nativeListener : google.maps.event[`addListener${isOnce ? 'Once' : ''}`];
